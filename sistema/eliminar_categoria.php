<?php
    ...
    if(isset($_POST['eliminar'])){
        ...
        $sql_eliminar = mysqli_query($conexion, "DELETE FROM categoria WHERE id_categoria='$id'");

        if($sql_eliminar){ ?>
            <div class="alert alert-primary alert-dismissible fade show" role="alert">
                <strong>Correcto!</strong> La categoría se borró con éxito!.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
        <?php }else{ ?>
            <div class="alert alert-danger alert-dismissible fade show" role="alert">
                <strong>Error!</strong> Error al borrar la categoría!.
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
        <?php }
    }
        ...
        <form action="" method="post" class="bg-white p-2 mt-2 border border-primary border-3">
            ...
            <div class="mb-3 d-grid gap-3">
                <input type="submit" value="Eliminar" name="eliminar" class="form-control btn btn-danger"
                       onclick="return confirm('¿Desea eliminar la categoría seleccionada?');">
